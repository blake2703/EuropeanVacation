...
#include "ui_adminchangefoodprice.h"

adminChangeFoodPrice::adminChangeFoodPrice(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::adminChangeFoodPrice)
{
    ui->setupUi(this);
    setupPage();
}

adminChangeFoodPrice::~adminChangeFoodPrice()
{
    delete ui;
}

void adminChangeFoodPrice::setupPage()
{
...
void adminChangeFoodPrice::on_updateButton_clicked()
{
       data.close();
...
void adminChangeFoodPrice::on_showAll_clicked()
{
    data.close();
    data.removeDatabase("first");
