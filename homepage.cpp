...
void homepage::on_userLoginButton_clicked()
{
    //Allow our Qstring var to match user input
    QString username = ui->userUsernameEdit->text();
        ...
        else
        {
            ...
            if (count == 1)
            {
                ...
                else
                {
                    ...
                    QMessageBox myBox;
                    myBox.setFont(newFont);
                    myBox.setText("Username and password incorrect.");
                    myBox.setInformativeText("Click yes to make a new account or cancel to try again.");
                    myBox.setStandardButtons(QMessageBox::Yes | QMessageBox::Cancel);
                    myBox.setDefaultButton(QMessageBox::Yes);
                    myBox.setStyleSheet("color: white");
                    myBox.setBaseSize(QSize(500, 150));
                    ...
                    if (yes == QMessageBox::Yes)
                    {
                        //make new addition page
                        userAddUser newUser;
                        newUser.setModal(true);
                        newUser.showMaximized();
                        newUser.exec();
                    }
                    else
                    {
